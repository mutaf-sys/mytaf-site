  ...
  return (
    <section className="mx-auto grid min-h-screen max-w-[1440px] items-center gap-12 px-4 pb-14 pt-28 sm:px-6 sm:pb-16 sm:pt-32 lg:grid-cols-2 lg:gap-14 lg:px-12 lg:pt-40">
      ...
      <div className="mx-auto w-full max-w-2xl lg:mx-0">
        <p className="mb-4 max-w-[280px] text-[10px] uppercase leading-5 tracking-[0.26em] text-brass sm:mb-5 sm:max-w-none sm:text-sm sm:tracking-[0.32em]">
          Санкт-Петербург · Более 10 лет опыта
        </p>

        <h1 className="font-heading text-[42px] leading-[1.02] text-foreground min-[390px]:text-[46px] sm:text-6xl lg:text-7xl">
          Историческая латунная фурнитура ручной работы
        </h1>

        <p className="mt-6 max-w-xl text-[15px] leading-7 text-muted sm:mt-8 sm:text-lg sm:leading-8">
        ...
        <div className="mt-8 grid gap-3 sm:mt-10 sm:flex sm:flex-wrap sm:gap-4">
          ...
          <a
            href="#contacts"
            className="flex w-full items-center justify-center border border-border px-6 py-4 text-sm uppercase tracking-[0.18em] text-brass-dark transition duration-300 hover:border-brass hover:bg-brass/10 sm:w-auto sm:px-8"
          >
            Обсудить проект
          </a>
        </div>

        <div className="mt-10 grid grid-cols-2 gap-3 border-t border-border pt-6 sm:mt-14 sm:max-w-xs sm:gap-10 sm:pt-8">
          <div>
            <p className="font-heading text-2xl text-brass sm:text-3xl">
              10+
            </p>
            <p className="mt-1 text-[11px] leading-4 text-muted sm:text-sm">
              лет опыта
            </p>
          </div>
      ...
      >
        <div className="absolute -inset-2 rounded-[30px] border border-brass/25 sm:-inset-4 sm:rounded-[42px]" />
